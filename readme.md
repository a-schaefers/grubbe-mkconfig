...
befallback on the other hand, is nothing special really. It destroys a boot env called "fallback" if it exists and then creates "fallback" again. The use case for this is for a situation where you are about to run a "risky" command-- first manually run "befallback", and then you know you have a safe and current boot environment to reboot into on doomsday.
